...
LLAMA_LIKE = ("llama", "Yi", "mistral", "mixtral", "gemma", "cohere", "qwen2", "qwen3_moe")
MOE_TYPES = ("mixtral", "qwen3_moe")
...
def load_dequantized_model(model, load_path):
    ...
    model.load_state_dict(
        torch.load(os.path.join(load_path, "not_quantized_weights.pt"), weights_only=False),
        strict=False,
    )
    return model
...
def load_quantized_model(model, load_path):
    ...
    for layer_index in range(len(model.model.layers)):
        model.model.layers[layer_index] = torch.load(
            ...
            weights_only=False,  # Required for PyTorch 2.6+ to load model layers with class objects
        )
...
def get_layers_prefix(config: transformers.PretrainedConfig) -> str:
    if config.model_type in ("llama", "mistral", "mixtral", "gemma", "qwen2", "qwen3_moe"):
        return "model.layers"
